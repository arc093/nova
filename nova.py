...
from ..community.core.edit import edit
import re
from abc import ABC, abstractmethod

from .novaHelpers import generate_payload, notify, log_to_csv, remove_wrapper
...
@mod.action_class
class UserActions:
    def command_match_2(command_to_process: str):
        """Search for relevant talon commands"""
        print('command match 2 called')
        command_list = ""
        active_ctxs = str(registry.active_contexts())
        talon_files = re.findall(r'Context\([^()]*\.talon\)', active_ctxs)
        print(talon_files)
        print(str(registry.active_contexts()))
        ...
        Here is a list of the possible commands, seperated by \n: {command_to_process}
        """
        # print(command_list)
    ...
    class CommandGetter(ABC):
        ...
        def get_commands(self) -> str:
            """Abstract method to get commands as a formatted string."""
            pass

    class SimpleCommandGetter(CommandGetter):
        def get_commands(self) -> str:
    ...
    def command_match(text_to_process: str, command_getter: CommandGetter) -> str:
        ...
        command_list=command_getter.get_commands()
        prompt = f"""
        Respond with the most likely command, no other text. 
        Here is a list of the possible commands: {command_list}
        """
        text_to_process = 'how do i ' + text_to_process
        response = gpt_query(prompt, text_to_process)
        log_to_csv(text_to_process, response)
        try:
            actions.mimic(response)
        except Exception as e:
            # If actions.mimic fails, catch the error and ignore it 
            log_to_csv(response, e)
        return response
...
user.command_match("help",user.SimpleCommandGetter)